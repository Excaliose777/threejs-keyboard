...
type SwitchProps = React.ComponentPropsWithoutRef<"group"> & {
...
export function Switch({ color, hexColor, ...restProps }: SwitchProps) {
  ...
  const handlePointerDown = (event: ThreeEvent<PointerEvent>) => {
    ...
    gsap.killTweensOf([switchGroup.rotation, stem.position]);
  ...
  return (
    <group {...restProps}>
      ...
      <group scale={10} rotation={[Math.PI / 2, 0, 0]} ref={switchGroupRef}>
        {/* Switch housing */}
        <mesh
          castShadow
          receiveShadow
